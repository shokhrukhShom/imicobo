...
final class MicManager: NSObject, ObservableObject {
    ...
    @Published var muted = false {
        didSet { audioTrack?.isEnabled = !muted; sendEffects(); updateNowPlaying() }
    }
    ...
    // MARK: - Lock screen / Control Center mute
    /// Maps the media transport to the mic: Pause = muted, Play = live. Because
    /// the app keeps running with the screen off, you can toggle your mic from
    /// the lock screen or Control Center without unlocking.
    private func setupRemoteCommands() {
        let c = MPRemoteCommandCenter.shared()
        c.playCommand.isEnabled = true
        c.pauseCommand.isEnabled = true
        c.togglePlayPauseCommand.isEnabled = true
        // Commands we don't use — disable so they don't clutter the lock screen.
        c.nextTrackCommand.isEnabled = false
        c.previousTrackCommand.isEnabled = false
        c.playCommand.addTarget  { [weak self] _ in self?.remoteSet(false); return .success }
        c.pauseCommand.addTarget { [weak self] _ in self?.remoteSet(true);  return .success }
        c.togglePlayPauseCommand.addTarget { [weak self] _ in self?.remoteToggle(); return .success }
    }
    nonisolated private func remoteSet(_ value: Bool) { Task { @MainActor in self.muted = value } }
    nonisolated private func remoteToggle()          { Task { @MainActor in self.muted.toggle() } }

    /// The lock-screen "Now Playing" card — only while live. Play/pause state
    /// mirrors mute so the button shows the right icon.
    private func updateNowPlaying() {
        guard phase == .live else { clearNowPlaying(); return }
        MPNowPlayingInfoCenter.default().nowPlayingInfo = [
            MPMediaItemPropertyTitle:  muted ? "Muted" : "Live — you're on the screen",
            MPMediaItemPropertyArtist: "iMicobo",
    ...
    private func setBitrate(_ kbps: Int) {
        ...
        for e in p.encodings {
            e.maxBitrateBps = NSNumber(value: kbps * 1000)
            // Mark the mic stream as high-priority voice traffic (DSCP EF).
            // Routers with WMM (Wi-Fi QoS, on by default) then put it in the
            // low-latency Voice queue instead of best-effort — less Wi-Fi jitter.
...
extension MicManager: RTCPeerConnectionDelegate {
    ...
    nonisolated func peerConnection(_ pc: RTCPeerConnection, didChange s: RTCIceConnectionState) {
        Task { @MainActor in
            switch s {
            case .connected, .completed: self.phase = .live; self.updateNowPlaying()
